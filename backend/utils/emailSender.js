...
const sendInitialVerificationEmail = (email, token) => {
  ...
  const mailOptions = {
    ...
    html: `
        ...
        <p>To verify your account, simply click on the following link: <a href="${process.env.API_URL}/auth/verify?token=${token}">Verify Your Account</a></p>
        ...
        <p>If more than 24 hours have passed since you received this email and you have not verified your account, don't worry! You can request a new verification link. Simply click here: <a href="${process.env.CLIENT_URL}/verification-results?status=expired&email=${email}">Request New Verification Link</a></p>
...
const sendResendVerificationEmail = (email, token) => {
  ...
  const mailOptions = {
    ...
    html: `
        ...
        <p><a href="${process.env.API_URL}/auth/update-verification?token=${token}">Verify Your Account</a></p>
